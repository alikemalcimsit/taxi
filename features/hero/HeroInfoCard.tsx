...
export default function HeroInfoCard() {
  ...
  return (
    <>
      <Script id="ld-json-hero-card" type="application/ld+json" strategy="afterInteractive" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} />

      <div
        className="mx-auto w-[min(760px,92%)] rounded-3xl bg-[#ECEEF1] text-black px-4 md:px-5 py-9 md:py-16 border border-black/5 shadow-[0_20px_50px_rgba(0,0,0,.15)] motion-safe:opacity-0 motion-safe:animate-[fadeUp_.4s_ease-out_forwards]"
        role="complementary"
      ...
      >
        ...
        <div className="grid items-center md:items-stretch gap-5 md:gap-6 md:grid-cols-[1fr_auto_auto]">
          ...
          {/* ORTA: Google puanı */}
          <div
            className="flex items-center gap-3 md:justify-self-center md:pl-5 md:border-l md:border-black/10 motion-safe:opacity-0 motion-safe:animate-[fadeUp_.35s_ease-out_forwards]"
          ...
          >
            <span className="grid place-items-center w-10 h-10 rounded-full bg-[#2B2B2B] ring-2 ring-black/10 shrink-0">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="#FFC000" aria-hidden><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>
            </span>
            <div className="leading-tight">
              <div className="font-extrabold text-[18px]"><span itemProp="ratingValue">4.9</span> / <span itemProp="bestRating">5</span> <span className="font-semibold">Google</span></div>
              <div className="text-xs text-black/60"><span itemProp="reviewCount">120</span>+ değerlendirme</div>
          ...
          {/* SAĞ: CTA’lar */}
          <div
            className="flex items-center gap-2.5 md:justify-self-end motion-safe:opacity-0 motion-safe:animate-[fadeUp_.35s_ease-out_forwards]"
...
function FeaturePill({ icon, children, index = 0 }: { icon: "check" | "bolt" | "clock"; children: React.ReactNode; index?: number; }) {
  const Icon = () => {
    if (icon === "check") return (<svg width="14" height="14" viewBox="0 0 24 24" fill="#FFC000" aria-hidden><path d="M9 16.17l-3.5-3.5-1.41 1.41L9 19 20.5 7.5 19.09 6.09z" /></svg>);
    if (icon === "bolt") return (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#FFC000" strokeWidth="2" aria-hidden><path d="M13 3l-1 9h9l-9 9 1-9H4l9-9z" /></svg>);
    return (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#FFC000" strokeWidth="2" aria-hidden><path d="M12 8v5l3 2" /><circle cx="12" cy="12" r="9" /></svg>);
  };

  return (
    <span
      className="inline-flex items-center gap-2 rounded-full bg-[#2B2B2B] text-white/90 ring-1 ring-black/10 px-3 py-1.5 text-[13px] transition will-change-transform hover:scale-[1.02] motion-safe:opacity-0 motion-safe:animate-[fadeUp_.28s_ease-out_forwards]"
